...
mod tests {
    ...
    fn full_ot() {
        ...
        let mut ot_sender = OTSender::new(messages);
        let mut ot_receiver = OTReceiver::<KEY_SIZE>::new(true);

        let first_msg = ot_sender.gen_first_message().unwrap();
        let second_msg = ot_receiver.handle_first_sender_message(first_msg).unwrap();
        let third_msg = ot_sender.handle_receiver_message(second_msg).unwrap();
